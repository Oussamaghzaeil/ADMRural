...
@Component({
  selector: 'app-editmedicaodearea',
  templateUrl: './editmedicaodearea.page.html',
  styleUrls: ['./editmedicaodearea.page.scss'],
})
export class EditmedicaodeareaPage implements OnInit {
  ...
  Map() {
    ...
    this.navCtrl.navigateRoot('/editmapamedicaodeareas');
  }
  ionViewWillLeave() {
  }
  ionViewDidLeave() {
    ...
    this.Authorizer.QueryStoreProc('Executar', 'spMedicao', params).then(res => {
      ...
      try {
        if (resultado.success) {
          this.alertService.presentAlert({ pTitle: 'Salvando Medição de área...', pSubtitle: '', pMessage: 'Operação realizada com sucesso!' });
          this.goBack();
        } else {
