...
@Component({
  selector: 'app-editpontosdeinteresse',
  templateUrl: './editpontosdeinteresse.page.html',
  styleUrls: ['./editpontosdeinteresse.page.scss'],
})
export class EditpontosdeinteressePage implements OnInit {
  ...
  Map() {
     sessionStorage.setItem("Codigoitem", btoa(this.codigo ))
      this.navCtrl.navigateRoot('/mapapontosdeinteresseedit');
  }
  ionViewWillEnter() {
    ...
    this.Authorizer.QueryStoreProc('Executar', 'spPontosdeInteresse', params).then(res => {
      ...
      try {
        if (resultado.success) {
          ...
          let C: any = JSON.parse(resultado.results)[0].UF;
          if (typeof (C) == 'undefined')
            C = '';
          if (C == null)
            C = '';
          if (C != '')
            this.CodigoFazenda = C;
