...
@Component({
  selector: 'app-novoitem',
  templateUrl: './novoitem.page.html',
  styleUrls: ['./novoitem.page.scss'],
})
export class NovoitemPage implements OnInit {
  ...
  goInsertUnidad(form: NgForm) {
    ...
    this.router.navigate(['/createunidade', this.TipoSelected]);
  }
  ...
  Delete() {
    ...
    this.Authorizer.QueryStoreProc('Executar', 'spEstoque', params).then(res => {
      this.alertService.presentAlert({ pTitle: 'Operação cancelada...', pSubtitle: '', pMessage: 'Operação realizada com sucesso!' });
      // this.alertService.showLoader(resultado.message, 500);
      this.goBack();
    });
  }
    ...
    this.Authorizer.QueryStoreProc('Executar', 'spEstoque', params).then(res => {
      ...
      try {
        if (resultado.success) {
          ...
          this.horadaentrada = JSON.parse(resultado.results)[0].HoradeEntrada;
