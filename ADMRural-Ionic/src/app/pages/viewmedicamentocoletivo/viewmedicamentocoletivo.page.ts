...
@Component({
  selector: 'app-viewmedicamentocoletivo',
  templateUrl: './viewmedicamentocoletivo.page.html',
  styleUrls: ['./viewmedicamentocoletivo.page.scss'],
})
export class ViewmedicamentocoletivoPage implements OnInit {
  Datainicio: Date;
  doses: number;
  Medicacao: string;
  repitir: number;
  ...
  MostraDados() {
    ...
    let params = {
      'StatusCRUD': 'Monstradados_Medicamento_Coletivo',
  ...
  delete(Codigomedicamento) {
    ...
    let params = {
      'StatusCRUD': 'Delete_Medicamento_Coletivo',
      'formValues': Codigomedicamento,
      'CodigoUsuarioSistema': JSON.parse(sessionStorage.getItem("SessionUser"))[0].CodigoUsuario,
      'Hashkey': sessionStorage.getItem("SessionHashkey"),
    };
