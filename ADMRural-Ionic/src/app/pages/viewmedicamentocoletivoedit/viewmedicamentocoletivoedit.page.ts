...
@Component({
  selector: 'app-viewmedicamentocoletivoedit',
  templateUrl: './viewmedicamentocoletivoedit.page.html',
  styleUrls: ['./viewmedicamentocoletivoedit.page.scss'],
})
export class ViewmedicamentocoletivoeditPage implements OnInit {
  Datainicio: Date;
  doses: number;
  Medicacao: string;
  repitir: number;
  ...
  MostraDados() {
    ...
    let params = {
      'StatusCRUD': 'Monstradados_Medicamento_coletivo_edit',
  ...
  delete(Codigomedicamento) {
    ...
    let params = {
      'StatusCRUD': 'Delete_Medicamento_coletivo_edit',
      'formValues': Codigomedicamento,
      'CodigoUsuarioSistema': JSON.parse(sessionStorage.getItem("SessionUser"))[0].CodigoUsuario,
      'Hashkey': sessionStorage.getItem("SessionHashkey"),
    };
