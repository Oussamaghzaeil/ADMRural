...
export class NovomedicaodeareaPage implements OnInit {
  ...
  Map() {
    if (this.incluirPronto == true) {
      ...
      this.alertService.presentAlert({ pTitle: 'ATENÇÃO', pSubtitle: 'Medição de área', pMessage: 'Salve o formulário antes de visualizar o mapa!' })
    }
  }
  ionViewWillLeave() {
  }
  ...
  getCoordinates() {
    if (this.Salvando == false) {
      this.alertService.presentAlert({ pTitle: 'ATENÇÃO', pSubtitle: 'Medição de área', pMessage: 'Salve o formulário antes de incluir as coordenadas!' });
    }
    else
