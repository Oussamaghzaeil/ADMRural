...
@Component({
  selector: 'app-viewmorteestoquesaida',
  templateUrl: './viewmorteestoquesaida.page.html',
  styleUrls: ['./viewmorteestoquesaida.page.scss'],
})
export class ViewmorteestoquesaidaPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.platform.backButton.subscribe(() => {
      this.navCtrl.navigateRoot('/saidaestoqueanimaismorte');
    })
  }
  goBack() {
    this.navCtrl.navigateRoot('/saidaestoqueanimaismorte');
  ...
  MostraDadosoutro() {
    ...
    console.log('vlue of CodigoAnimal :', atob(CodigoAnimal));
    let params = {
      'StatusCRUD': 'Pesquisar_animal_morte',
      'formValues': atob(CodigoAnimal),
      'CodigoUsuarioSistema': JSON.parse(sessionStorage.getItem("SessionUser"))[0].CodigoUsuario,
      'Hashkey': sessionStorage.getItem("SessionHashkey"),
    };
    ...
    this.Authorizer.QueryStoreProc('Executar', 'spAnimal', params).then(res => {
      ...
      try {
        if (resultado.success) {
          this.ValorAlimentacao = JSON.parse(resultado.results)[0].ValorAlimentacao;
        }
      } catch (err) {
