...
@Component({
  selector: 'app-viewcoletivotratamento',
  templateUrl: './viewcoletivotratamento.page.html',
  styleUrls: ['./viewcoletivotratamento.page.scss'],
})
export class ViewcoletivotratamentoPage implements OnInit {
  ...
  goToAplicadoColetivo() {
    ...
    this.navCtrl.navigateRoot('/aplicacadomedicamento-coletivo');
  }
    ...
    this.Authorizer.QueryStoreProc('Executar', 'spColetivo', params).then(res => {
      ...
      try {
        if (resultado.success) {
          ...
          this.alertService.presentAlert({ pTitle: 'ATENÇÃO', pSubtitle: 'Tratamento Coletivo', pMessage: 'Nenhum dados' });
        }
      } catch (err) {
      }
    });
  ...
  MostraDadosaplicacao() {
    ...
    console.log('vlue of Codigoitem :', Codigoitem);
    let params = {
      'StatusCRUD': 'Monstradados_Apli_Coletivo_Edit',
      'formValues': Codigoitem,
      'CodigoUsuarioSistema': JSON.parse(sessionStorage.getItem("SessionUser"))[0].CodigoUsuario,
      'Hashkey': sessionStorage.getItem("SessionHashkey"),
    };
